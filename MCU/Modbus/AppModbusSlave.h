...
public:
	...
	static bool setSetting(std::map<u16, std::string>& data, std::string device, std::string section, TSlotDataHandler callback);
private:
	static u8 CreateWriteCmd(u8* a, TWriteCmdSrc CmdSrc);
	static u8 get0x10WriteRegCmd(u8* a, TWriteCmdSrc& Src);
	static u8 get0x16MaskWriteRegisterCmd(u8* a, TWriteCmdSrc& Src);
