...
void HandlerSubscribers::send(Slot& slot /*TODO аргумент "U1/RAM/"*/) {
	...
	if (Handlers.count(slot.Device)) {
		std::map<std::string, std::function<void(bool)>> sections = Handlers.at(slot.Device);
		if (sections.count(slot.Section)) {
			std::function<void(bool)> handler = sections.at(slot.Section);
			handler(false);
		}
	}
}